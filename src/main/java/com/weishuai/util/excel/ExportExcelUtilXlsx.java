...
public class ExportExcelUtilXlsx {
    ...
    public static void export(String excelSuffix, List<List<String>> valList, List<String> rowNames, String titlePrefix, HttpServletResponse response) {
        ...
        XSSFCellStyle titleStyle = configStyle(workbook, (short) 15, IndexedColors.PALE_BLUE, FillPatternType.SOLID_FOREGROUND);
        cellTitle.setCellStyle(titleStyle);
        cellTitle.setCellValue("表头标题");
        //设置表格列名范围、填充样式、值
        CellRangeAddress cellRangeAddress = new CellRangeAddress(0, 0, 0, (rowNames.size() - 1));
        sheet.addMergedRegion(cellRangeAddress);
        XSSFCellStyle columnStyle = configStyle(workbook, (short) 15, IndexedColors.LIGHT_BLUE, FillPatternType.SOLID_FOREGROUND);
        setColumnTopVal(rowNames, sheet, columnStyle);
        //设置sheet内容数据
        XSSFCellStyle cellStyle = configStyle(workbook, (short) 12, IndexedColors.BLACK1, FillPatternType.NO_FILL);
        ...
        String headStr = null;
        try {
            headStr = "attachment; filename=" + URLEncoder.encode(fileName, Charsets.UTF_8.name());
        ...
        if (Objects.nonNull(response)) {
            ...
            //下载到本地
            try {
                FileOutputStream out = new FileOutputStream("/Users/ws/Desktop/" + headStr);
                workbook.write(out);
            } catch (IOException e) {
                log.error(SYSTEM_EXPORT_FAIL_EXCEPTION.getReason(), e);
                CommonResult.fail(SYSTEM_EXPORT_FAIL_EXCEPTION);
    ...
    private static void setVal(List<List<String>> valList, XSSFSheet sheet, XSSFCellStyle style) {
    ...
    private static void setColumnTopVal(List<String> rowNames, XSSFSheet sheet, XSSFCellStyle columnTitleStyle) {
        // 定义所需列数
        int columnNum = rowNames.size();
     ...
     * 设置列头单元格样式
     */
    public static XSSFCellStyle configStyle(XSSFWorkbook workbook, short fontSize, IndexedColors indexColorType, FillPatternType fillPatternType) {
    ...
    private static void setPublicStyle(XSSFFont font, short fontSize, XSSFCellStyle style) {
        //设置字体大小
